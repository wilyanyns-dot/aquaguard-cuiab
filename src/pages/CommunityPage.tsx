...
const tips = [
  { id: 1, author: "Maria Silva", bairro: "CPA II", title: "Água da Máquina de Lavar", desc: "Uso a água do último enxágue para lavar a calçada e o quintal. Economizo quase 100 litros por semana!", likes: 140, tags: ["Economia de Água", "ReparoFácil"], badge: "Morador Engajado" },
  { id: 2, author: "João Santos", bairro: "Pedra 90", title: "Captação de Água da Chuva", desc: "Instalei calhas e um reservatório simples. Uso para regar plantas e lavar o carro.", likes: 89, tags: ["Reuso de Chuva", "HortaCaseira"], badge: "Morador Engajado" },
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="px-5 -mt-3">
        ...
        <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
          {categories.map((c) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-[10px] font-display font-medium whitespace-nowrap transition-colors ${
        ...
        <div className="space-y-3">
          {tips
            .filter((t) => !filter || t.tags.includes(filter))
            .map((tip, i) => (
              <motion.div
                key={tip.id}
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {tip.badge && (
                    <span className="ml-auto px-2 py-0.5 rounded-full bg-verde-sucesso/10 text-verde-sucesso text-[9px] font-display font-medium">{tip.badge}</span>
                  )}
                </div>
                ...
                <div className="flex gap-1 mb-3">
                  {tip.tags.map((tag) => (
                    <span key={tag} className="text-[9px] font-body text-primary bg-primary/5 px-2 py-0.5 rounded-full">#{tag}</span>
                  ))}
                </div>
                {/* Actions */}
                <div className="flex items-center gap-4">
                  <button className="flex items-center gap-1 text-xs font-body text-cinza-medio hover:text-verde-sucesso transition-colors">
                    <ThumbsUp className="w-4 h-4" strokeWidth={1.5} /> {tip.likes} Útil
                  </button>
                  <button className="flex items-center gap-1 text-xs font-body text-cinza-medio hover:text-primary transition-colors">
                    <BookmarkPlus className="w-4 h-4" strokeWidth={1.5} /> Vou testar
                  </button>
                  <button className="ml-auto flex items-center gap-1 text-xs font-body text-cinza-medio hover:text-primary transition-colors"
                    onClick={() => navigate("/conquistas")}
                  >
                    🏆
                  </button>
                  <button className="flex items-center gap-1 text-xs font-body text-cinza-medio">
